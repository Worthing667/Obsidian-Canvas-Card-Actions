...
定位依赖 Obsidian Canvas 运行时的内部方法；如果当前版本或布局下无法调用定位方法，插件会退回到选中目标卡片和临时高亮反馈。
...
- Canvas 中优先对当前命中的渲染文本做临时高亮；如果无法把原始文本命中映射到渲染 DOM，则回退到卡片级外框/脉冲反馈
- Canvas 正文高亮只包裹当前渲染 DOM，不写入 `.canvas` 数据；关闭面板、切换查询或跳转到其它命中时会清理旧高亮
