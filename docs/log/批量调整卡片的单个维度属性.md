单个数值的批量调整：
只输入宽度：所有卡片宽度统一，高度保持各自原值
只输入高度：所有卡片高度统一，宽度保持各自原值
同时输入：宽度和高度都统一

修改代码，保留"锁定宽高比"功能的同时，添加单维度调整功能：
...
export class CardPropertiesModal extends Modal {
  ...
  private createBatchActions(container: HTMLElement): void {
    ...
    // 添加功能说明
    const functionDesc = customSizeGroup.createDiv({ cls: "function-description" });
    functionDesc.innerHTML = `
      <div class="desc-item">• 只输入宽度：统一宽度，保持各自高度</div>
      <div class="desc-item">• 只输入高度：统一高度，保持各自宽度</div>
      <div class="desc-item">• 同时输入：统一所有尺寸</div>
    `;
    ...
    this.widthInput = widthGroup.createEl("input", {
      type: "number",
      value: "",
      attr: { min: "50", max: "2000", placeholder: "留空保持原值" }
    }) as HTMLInputElement;
...
位置范围: X: ${stats.minX}-${stats.maxX}, Y: ${stats.minY}-${stats.maxY}`;
  ...
  private async unifyWidthOnly(width: number): Promise<void> {
    try {
      // 调用 CardService 的单维度调整方法
      // 注意：这里假设 CardService 有相应的方法，如果没有需要添加
      if (typeof this.cardService.unifyCardWidth === 'function') {
        await this.cardService.unifyCardWidth(this.cards, width);
      } else {
        // 如果 CardService 没有相应方法，可以手动调整每个卡片
        for (const card of this.cards) {
          const data = card.getData();
          card.setData({ ...data, width: width });
        }
        // 触发重新渲染
        this.cards[0]?.canvas?.requestSave?.();
      }
      
      new Notice(`已将所有卡片宽度统一为 ${width}px，高度保持不变`);
      this.close();
    } catch (error) {
  ...
  private async unifyHeightOnly(height: number): Promise<void> {
    try {
      // 调用 CardService 的单维度调整方法
      // 注意：这里假设 CardService 有相应的方法，如果没有需要添加
      if (typeof this.cardService.unifyCardHeight === 'function') {
        await this.cardService.unifyCardHeight(this.cards, height);
      } else {
        // 如果 CardService 没有相应方法，可以手动调整每个卡片
        for (const card of this.cards) {
          const data = card.getData();
          card.setData({ ...data, height: height });
        }
        // 触发重新渲染
        this.cards[0]?.canvas?.requestSave?.();
      }
      
      new Notice(`已将所有卡片高度统一为 ${height}px，宽度保持不变`);
      this.close();
    } catch (error) {
...
### 1. **单维度批量调整**（主要新功能）
- **只输入宽度**：所有卡片宽度统一，但保持各自原有的高度
- **只输入高度**：所有卡片高度统一，但保持各自原有的宽度  
- **同时输入**：所有卡片的宽度和高度都统一

### 2. **保留的锁定宽高比功能**
- 勾选"锁定宽高比（等比例调整）"后，输入一个值会自动计算另一个值
- 所有卡片按相同比例调整

## 🎨 界面改进

1. **功能说明**：在输入区域上方添加了清晰的使用说明
2. **更好的占位符**：输入框显示"留空保持原值"提示
3. **默认空白**：输入框默认为空，用户可以选择性填写

## 💡 使用场景示例

- **统一卡片宽度**：比如让所有卡片都是 400px 宽，但保持原有高度差异
- **统一卡片高度**：比如让一行卡片都是 200px 高，但保持原有宽度差异
- **完全统一**：让所有卡片都变成 400×200px
- **等比例缩放**：勾选锁定宽高比，输入宽度 600px，系统自动按比例计算高度

## ⚠️ 注意事项

代码中我添加了 `unifyWidthOnly` 和 `unifyHeightOnly` 方法，但假设 CardService 有对应的 `unifyCardWidth` 和 `unifyCardHeight` 方法。如果 CardService 中没有这些方法，我提供了备用的手动调整逻辑，或者您需要在 CardService 中添加这些方法。