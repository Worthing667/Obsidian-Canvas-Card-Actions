...
![对比按位置排序和按标记排序](Demo/readme/sort-comparison.png)

位置排序跟随画布上的视觉排布。标记排序跟随 `1`、`2.1`、`10.3.2` 这类数字层级标记，所以卡片位置被挪动后，输出顺序仍然可以保持稳定。

演示视频：[按位置排序和按标记排序的区别](Demo/预览卡片组：按位置排序和按标记排序的区别.mp4)

### 标记卡片

![批量添加或移除 Canvas 卡片标记](Demo/readme/badge-management.png)

可以给单张卡片添加或编辑数字标记，也可以按当前画布位置给选中的卡片批量编号。标记可以独立控制拼合顺序，不必强制维持卡片的视觉位置。
